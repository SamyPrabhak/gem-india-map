...
export function GroupTabs({ active, onChange }: Props) {
  return (
    <div className="inline-flex flex-wrap items-center gap-1 rounded-full border border-[color:var(--gold-border)] bg-[color:var(--ivory)] p-1 shadow-sm">
      {TABS.map((t) => {
            ...
            className={`relative rounded-full px-5 py-2 text-sm font-medium tracking-wide transition ${
              isActive
                ? "bg-[color:var(--gold)] text-[color:var(--ink)] shadow"
                 : "text-[color:var(--text-secondary)] hover:text-[color:var(--ink)]"
            }`}
          >
            {t.label}
          </button>
